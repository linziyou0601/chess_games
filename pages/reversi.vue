<template>
  <div id="app">
                ...
                <b-list-group>
                  ...
                  >
                    <b-row class="flex-nowrap">
                      ...
                      <b-col cols="auto text-truncate">{{ userNameMap[rData.black] || rData.black }}</b-col>
                      <b-col cols="auto"><fa :icon="['fas', 'mitten']" class="dialog-icon" /></b-col>
                      <b-col cols="auto text-truncate">{{ userNameMap[rData.black] || rData.white }}</b-col>
                    </b-row>
                  </b-list-group-item>
                </b-list-group>
              </div>
    ...
    <AlertDialog />
    <ReversiInviteDialog />
    <ReversiGameRoomDialog />
    <ReversiGameEndDialog />
    <ReversiLeaveRoomDialog />
    <ReversiSignUpOrSignInDialog />
    <ReversiGameRoomWatcherDialog />
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    ...mapMutations(namespace, ['showSignUpOrSignInDialog']),
    // 發送訊息
    sendMessage() {
    ...
    async signUpOrSignIn(name, password, cb) {
      try {
        const result = await this.$axios.$post('/api/Reversi/signUpOrSignIn', { name, password })
        if (result.valid) {
    ...
    async fetchRank() {
      try {
        const users = await this.$axios.$post('/api/Reversi/findAll')
        this.ranks = users
      } catch(err) {
        console.log(err)
      }
    ...
    async updateCurrentUser() {
      try {
        const user = await this.$axios.$post('/api/Reversi/findByName', { name: this.currentUser.name })
